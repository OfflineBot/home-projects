...
func (s *Server) mountBlueprint(r fiber.Router) {
	r.Get("/export", requireOwner, func(c *fiber.Ctx) error {
		doc, err := blueprint.Export(c.UserContext(), s.Store, c.Query("group"))
		if err != nil {
	...
	r.Post("/import", requireOwner, func(c *fiber.Ctx) error {
		...
		result, err := applier.Apply(c.UserContext(), &doc, !apply)
		if err != nil {
			return httpx.Internal("the import stopped partway: %v", err).WithDetail(result)
		}
		if apply {
			s.Store.Audit(c.UserContext(), actor.UserID(), "blueprint.imported",
